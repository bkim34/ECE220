...
In this mp we are building a floorplanner that lays out a set of rectangular modules and optimizes the packing area.
We use binary tree structure to describe the rectangles and ground into a bigger area. The resulting tree is a left skewed
tree, meaning it grows down and to the left. */
...
if(ptr->left == NULL && ptr->right == NULL){ //checking if node is leave node through NULL
  return 1;
}
return 0;
}
...
if(ptr->left != NULL && ptr->right != NULL){ //checking if node is interna node through NULL
  return 1;
}
// TODO: (remember to modify the return value appropriately)
return 0;
...
if (a->left == b || a->right == b) { // check if b is a subtree of a
  return 1;
}
return is_in_subtree(a->left,b); // recursively check if b is subtree of a
...
int temp = (ptr->module)->w;         // use temp to swap w and h
...
assert(ptr->module == NULL && ptr->cutline != UNDEFINED_CUTLINE); // check module and cutline
if(ptr->cutline == V){
  ptr->cutline = H;      // swap V and H
}
else{
  ptr->cutline = V;
}
...
module_t *temp = a->module; // swap function with temp variable
a->module = b->module;
b->module = temp;
// TODO:
}
...
// TODO:
node_t* parent_a = a->parent; // create temp variables
node_t* parent_b = b->parent;
 
if(a->parent->left == a) { // check if a is left or right, and swap with b
   a->parent->left = b;
   b->parent = parent_a;
}
else{
...
if(parent_b->left == b) { // check if b is left or right, and swap with a
 parent_b->left = a;
 a->parent = parent_b;
}
else{
...
if(ptr->module != NULL){
 expression[*nth].module = ptr->module;        // if ptr->module exists, the expression is module and cutline is undefined
 expression[*nth].cutline = UNDEFINED_CUTLINE; // else the module is NULL and expression is cutline
}
else{
...
// TODO:
if (ptr==NULL)
{                   // base case checking if ptr is null
 return 0;
}
if (ptr->module!=NULL){
 return (ptr->module)->resource+get_total_resource(ptr->left)+get_total_resource(ptr->right); // recursive case returning either
}                                                                                              // left and right with resource or without
else
{
...
node_t* init_slicing_tree(node_t* par, int n) {
 ...
 if (n == (num_modules - 1)){        // base case checking last module
   ptr->module = modules + n;
   ptr->cutline = UNDEFINED_CUTLINE;
   return ptr;
 }
ptr->cutline = V;    // recursive case for ptr is cutline
ptr->module = NULL;
 
ptr->right = (node_t*)malloc(sizeof(node_t)); // right child
ptr->right->parent = ptr;
ptr->right->module = modules + n;
ptr->right->cutline = UNDEFINED_CUTLINE;
ptr->left = init_slicing_tree(ptr,n+1); // left child
return ptr;
